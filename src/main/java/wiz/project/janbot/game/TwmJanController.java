 ...
 * 台湾麻雀コントローラ (ソロ)
 */
class TwmJanController implements JanController {
    
    /**
     * コンストラクタ
     */
    public TwmJanController() {
    }
     ...
     * @param observer 監視者。
     */
    public TwmJanController(final Observer observer) {
        if (observer != null) {
    ...
    public void start(final List<JanPai> deck, final Map<Wind, Player> playerTable) throws JanException {
        ...
        synchronized (_GAME_INFO_LOCK) {
            ...
            _info.setWanPai(new WanPai(new ArrayList<>(deck.subList(deckSize - 16, deckSize))));
            
            // 配牌。台湾麻雀の手牌は16枚。
            _info.setHand(Wind.TON, new Hand(new ArrayList<JanPai>(deck.subList( 0, 16))));
            _info.setHand(Wind.NAN, new Hand(new ArrayList<JanPai>(deck.subList(16, 32))));
            _info.setHand(Wind.SHA, new Hand(new ArrayList<JanPai>(deck.subList(32, 48))));
            _info.setHand(Wind.PEI, new Hand(new ArrayList<JanPai>(deck.subList(48, 64))));
            _info.setDeckIndex(16 * 4);
            // 台湾麻雀は王牌、手牌が16枚なので、残り枚数は56枚 ※花牌を除く
            _info.setRemainCount(56);
    ...
    private void callKanAdd(final JanPai target) throws JanException {
        ...
        // 直前のツモ牌を手牌に加える
        hand.addTwmJanPai(_info.getActiveTsumo());
